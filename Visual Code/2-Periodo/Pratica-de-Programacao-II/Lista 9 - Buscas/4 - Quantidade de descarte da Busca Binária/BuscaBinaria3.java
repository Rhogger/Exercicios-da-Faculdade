...
public class BuscaBinaria3 {
  public static void main(String args[]) {
    ...
    int inicio = 0, meio, fim = vetor1.length - 1, i = 0, j = 0, encontrou = 0, num, it = 1, descarte = 0, it2, aux = 0;
    boolean div = false;
    ;
    ...
    for (j = 0; j < (vetor1.length / 2); i++) {

      if (div == true && descarte > 0 && it2 % 2 == 0) {
        System.out.printf("Iteracao %d: \n\nDescartou %d numeros!\n\n\n", it, descarte);
        it++;
        descarte = 0;
      } else if (div == false && descarte > 0 && it2 % 2 != 0) {
        System.out.printf("Iteracao %d: \n\nDescartou %d numeros!\n\n\n", it, descarte);
        it++;
        descarte = 0;
      }
      ...
      if (aux == 0) {
        for (i = inicio; i <= fim; i++) {
          ...
          if (num == vetor1[i]) {
            System.out.printf("O valor (%d) buscado foi encontrado na posicao %d .\n\nReturn: 1\n\n\n\n\n", num, i);
            encontrou++;
          }

          //Se não for o N° que estamos buscando, ele incrementa no "descarte"
          else if (num != vetor1[i]) {
            descarte++;
          }
          aux++;
        }
